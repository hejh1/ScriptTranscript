...
asian_table = PrettyTable(['File', 'Language', 'Accent', 'URL', 'diffViewer', 'wer', 'mer', 'wil', 'wip', 'cer', 'OpenAI wer', 'OpenAI mer', 'OpenAI wil', 'OpenAI wip', 'OpenAI cer'])
indian_table = PrettyTable(['File', 'Language', 'Accent', 'URL', 'diffViewer', 'wer', 'mer', 'wil', 'wip', 'cer', 'OpenAI wer', 'OpenAI mer', 'OpenAI wil', 'OpenAI wip', 'OpenAI cer'])
english_table = PrettyTable(['File', 'Language', 'Accent', 'URL', 'diffViewer', 'wer', 'mer', 'wil', 'wip', 'cer', 'OpenAI wer', 'OpenAI mer', 'OpenAI wil', 'OpenAI wip', 'OpenAI cer'])
    ...
    for txt_file in script_files:
        if txt_file in subtitle_files:
            ...
            row = [
                ...
                '<button onclick="diffViewer(\''+ reference_file +'\', \''+ hypothesis_file +'\')">diffView</button>',
                f"{wer:.2%}",
                f"{mer:.2%}",
                f"{wil:.2%}",
                f"{wip:.2%}",
...
<script>
    ...
    function diffViewer(reference_file, hypothesis_file) {
        Promise.all([
            fetch(reference_file).then(response => {
                if (!response.ok) throw new Error('Cannot load '+reference_file+': ' + response.statusText);
                return response.text();
            }),
            fetch(hypothesis_file).then(response => {
                if (!response.ok) throw new Error('Cannot load '+ hypothesis_file +': ' + response.statusText);
                return response.text();
            })
        ])
        .then(([referenceData, hypothesisData]) => {
            localStorage.setItem('transcriptData', referenceData);
            localStorage.setItem('subtitleData', hypothesisData);
            console.log('Contents stored in localStorage.');
