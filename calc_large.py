...
models = ["large-v3", "large-v3-q5_0", "large-v3-turbo", "large-v3-turbo-q5_0"]
html_tabs = """<div>
"""
html_tables = """"""
for mod in models:
    ...
    for accent_file in accent_files:
        ...
        script_path = accent_path + accent_file + "/Transcript-"+ mod + "/"
    ...
    asianTab = mod + "-asian"
    indianTab = mod + "-indian"
    englishTab = mod + "-english"
    html_tabs += f"""
         <div class="tab-button" onclick="openTab(event, '{asianTab}')">{asianTab}</div>
         <div class="tab-button" onclick="openTab(event, '{indianTab}')">{indianTab}</div>
         <div class="tab-button" onclick="openTab(event, '{englishTab}')">{englishTab}</div>
    """
    
    html_tables += f"""
    <div id="{asianTab}" class="tab">
    """
    html_tables += asian_table_text
    html_tables += """
    </div>
    """
    html_tables += f"""
    ...
    """
    html_tables += english_table_text
    html_tables += """
    </div>
    """
...
html_content += html_tabs
html_content += html_tables
# <div id="englishTab" class="tab">
# """
...
with open('audio_large-model_jiwer.html', 'w') as f:
